...
public class Brian2Writer extends ANeuroMLBaseWriter
{

	static String DEFAULT_POP = "OneComponentPop";

	public Brian2Writer(Lems lems) throws ModelFeatureSupportException, LEMSException, NeuroMLException
	{
		super(lems, Format.BRIAN);
	}

	public Brian2Writer(Lems lems, File outputFolder, String outputFileName) throws ModelFeatureSupportException, NeuroMLException, LEMSException
	{
				...
				if(outComp.getTypeName().equals("OutputFile"))
				{
					...
					for(Component colComp : outComp.getAllChildren())
					{
						if(colComp.getTypeName().equals("OutputColumn"))
						{
							...
							postRunSave.append(monitor + "." + l1.getVariable() + "[0] ");
				...
				if(dispComp.getTypeName().equals("Display"))
				{
					...
					for(Component lineComp : dispComp.getAllChildren())
					{
						if(lineComp.getTypeName().equals("Line"))
						{
							...
							postRunPlot.append("    " + plotId + ".plot(" + trace + "." + times + "/second," + trace + "." + l1.getVariable() + "[" + l1.getPopulationIndex()
	...
	public static void main(String[] args) throws Exception
	{
		...
		for(File lemsFile : lemsFiles) {
            ...
            Brian2Writer bw = new Brian2Writer(lems);

            String br = bw.getMainScript();

            File brFile = new File(lemsFile.getAbsolutePath().replaceAll(".xml", "_brian" +  "2"  + ".py"));
            ...
            bw = new Brian2Writer(lems);

            br = bw.getMainScript();

            brFile = new File(lemsFile.getAbsolutePath().replaceAll(".xml", "_brian" + "2" + ".py"));
            System.out.println("Writing to: " + brFile.getAbsolutePath());
